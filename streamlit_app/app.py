...
if btn_criar:
    ...
    res = requests.post(f"{"https://crud-fast-api-7vav.onrender.com"}/items/", json=payload)
...
if btn_atualizar and id_item > 0:
    payload = {"nome": nome, "preco": preco, "descricao": descricao}
    res = requests.put(f"{"https://crud-fast-api-7vav.onrender.com"}/items/{id_item}", json=payload)
    if res.status_code == 200:
...
if btn_deletar and id_item > 0:
    res = requests.delete(f"{"https://crud-fast-api-7vav.onrender.com"}/items/{id_item}")
    if res.status_code == 200:
...
try:
    resposta_get = requests.get(f"{"https://crud-fast-api-7vav.onrender.com"}/items/")
